...
public:
	...
	/* public functions */
	void begin(uint8_t port = RAIN); //Set board's pins
	void update(void);
	float getRain(); //Returns the rain in mm/h
...
private:
  ...
  static Rain *instance; //Instance used for interrupt; only the newest object will work
  
  /* private functions */
  uint32_t getLastRain(void);
  static void rainIRQ();
};