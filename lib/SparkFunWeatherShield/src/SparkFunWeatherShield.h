...
public:
  WeatherShield(uint8_t sda = SHIELD_SDA_PIN, uint8_t scl = SHIELD_SCL_PIN);
  ~WeatherShield();

  //Public Functions
  void begin(bool use_leds = false); // start I2C & sensors
  void loop(void);
  float readHumidity(); // get sensor's humidity
  float readTemperature(); // get sensor's temparature
  float readPressure(); // get sensor's pressure
  float getBatteryLevel(); //Returns the voltage of the raw pin based on the 3.3V rail
  float getLightLevel(); //Returns the voltage of the light sensor based on the 3.3V rail
  float getWindSpeed();
  float getWindSpeedKm();
  int getWindDirection();
  int decodeWindDirection(unsigned int adc);
  float getRain(); //Returns the rain in mm/s
